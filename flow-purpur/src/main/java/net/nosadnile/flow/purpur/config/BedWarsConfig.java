...
import net.nosadnile.flow.purpur.lang.English;
...
public class BedWarsConfig extends PluginConfig {
    ...
    public BedWarsConfig() {
        super(FlowPurpur.getPlugin(), "config", FlowPurpur.getPlugin().getDataFolder().getPath());
        ...
        yml.addDefault(ConfigPath.GENERAL_CONFIGURATION_ARENA_SELECTOR_STATUS_MATERIAL.replace("%path%", "skipped-slot"), String.valueOf(FlowPurpur.getMaterialAdapter().getForCurrent("STAINED_GLASS_PANE", "STAINED_GLASS_PANE", "BLACK_STAINED_GLASS_PANE")));
        ...
        String whatLang = "en";
        new English();
        File[] langs = new File("plugins/" + FlowPurpur.getPlugin().getDescription().getName() + "/Languages").listFiles();
        if (langs != null) {
            for (File f : langs) {
                if (f.isFile()) {
                    if (f.getName().startsWith("messages_") && f.getName().endsWith(".yml")) {
                        ...
                        if (LanguageManager.get().getLang(lang) == null)
                            new LanguageHelper(FlowPurpur.getPlugin(), lang);
                    }
                }
            }
        }

        Language def = LanguageManager.get().getLang(whatLang);
        ...
        for (String iso : yml.getStringList(ConfigPath.GENERAL_CONFIGURATION_DISABLED_LANGUAGES)) {
            Language l = LanguageManager.get().getLang(iso);
