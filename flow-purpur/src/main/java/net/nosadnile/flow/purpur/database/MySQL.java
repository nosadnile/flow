...
public class MySQL implements Database {
     ...
     */
    public MySQL() {
        this.host = FlowPurpur.config.getYml().getString("database.host");
        this.database = FlowPurpur.config.getYml().getString("database.database");
        this.user = FlowPurpur.config.getYml().getString("database.user");
        this.pass = FlowPurpur.config.getYml().getString("database.pass");
        this.port = FlowPurpur.config.getYml().getInt("database.port");
        this.ssl = FlowPurpur.config.getYml().getBoolean("database.ssl");
        if (!connect()) {
            FlowPurpur.setRemoteDatabase(new NoDatabase());
        }
    }
    ...
    public void init() {
        ...
        Bukkit.getScheduler().runTaskTimerAsynchronously(FlowPurpur.getPlugin(), new SessionKeeper(this), 20 * 60, 20 * 3600);
    }
    ...
    public void updateLocalCache(UUID uuid) {
        ...
        try (ResultSet rs = connection.createStatement().executeQuery("SELECT * FROM global_stats WHERE uuid = '" + uuid.toString() + "';")) {
            if (rs.next()) {
                StatsCache cs = FlowPurpur.getStatsCache();
