...
public class PacketSign {
     ...
     * Instantiate {@link SignAPI} and then use {@link SignAPI#createSign(Block)} (Block)}
     *
    ...
    public void refresh() {
        ...
        for (Player p : w.getPlayers()) {
            if (isInRange(p.getLocation().getBlockX(), p.getLocation().getBlockZ())) {
                SignAPI.signVersion.update(p, new Location(w, (int) loc.getX(), (int) loc.getY(), (int) loc.getZ()), getContent().apply(p));
            }
        }
    }
