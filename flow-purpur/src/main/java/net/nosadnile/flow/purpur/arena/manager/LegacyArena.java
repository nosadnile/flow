...
import net.nosadnile.flow.purpur.api.bedwars.*;
import net.nosadnile.flow.purpur.api.events.ArenaCacheUpdateEvent;
import net.nosadnile.flow.purpur.api.events.PlayerArenaJoinEvent;
import net.nosadnile.flow.purpur.api.events.PlayerReJoinEvent;
...
public class LegacyArena implements CachedArena {
    ...
    public LegacyArena(String remoteIdentifier, String server, @NotNull String group, String arenaName, ArenaStatus status, int maxPlayers, int currentPlayers, int maxInTeam) {
        ...
        FlowPurpur.getAPI().getLanguageUtil().saveIfNotExists(Messages.ARENA_DISPLAY_GROUP_PATH + getArenaGroup().toLowerCase(), group);
        FlowPurpur.getAPI().getLanguageUtil().saveIfNotExists(Messages.ARENA_DISPLAY_NAME_PATH + getArenaName().toLowerCase(), arenaName);
    }
    ...
    public void setStatus(ArenaStatus arenaStatus) {
        ...
        if (status != ArenaStatus.PLAYING) {
            FlowPurpur.getAPI().getArenaUtil().destroyReJoins(this);
        }
    }
    ...
    public boolean addSpectator(Player player, String targetPlayer) {
        ...
        player.sendPluginMessage(FlowPurpur.getPlugin(), "BungeeCord", out.toByteArray());
        return true;
    }
    ...
    public boolean addPlayer(Player player, String partyOwnerName) {
        ...
        if (getCurrentPlayers() >= getMaxPlayers() && !isVip(player)) {
            ...
            text.setClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, FlowPurpur.config.getYml().getString("storeLink")));
    ...
    public boolean reJoin(RemoteReJoin rj) {
        ...
        json.addProperty("type", "PLD");
        json.addProperty("uuid", player.getUniqueId().toString());
        json.addProperty("lang_iso", FlowPurpur.getAPI().getLanguageUtil().getPlayerLanguage(player).getIso());
