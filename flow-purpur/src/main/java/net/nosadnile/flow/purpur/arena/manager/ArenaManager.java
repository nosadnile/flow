...
import static net.nosadnile.flow.purpur.FlowPurpur.config;
...
public class ArenaManager implements BedWars.ArenaUtil {
    ...
    public boolean joinRandomFromGroup(@NotNull Player p, String group) {
        ...
        int amount = getParty().hasParty(p.getUniqueId()) ? getParty().getMembers(p.getUniqueId()).size() : 1;
    ...
    public void destroyReJoins(CachedArena arena) {
        ...
        for (Map.Entry<UUID, RemoteReJoin> rrj : RemoteReJoinManager.getRejoinByUUID().entrySet()) {
    ...
    public RemoteReJoin getReJoin(UUID player) {
        return RemoteReJoinManager.getReJoin(player);
    }
}
